...
//总体：
//您在HTML页上创建表单，并在表单的HTML标记中指定CGI脚本的名称，当用户单击表单上的Submit按钮时将调用该脚本。用户输入到表单中的值将会进行打包并发送到CGI脚本，
...
void * accept_request(void *tclient)
{
	...
	numchars = get_line(client, buf, sizeof(buf));//从client中读取指定大小数据到buf     GET /index2.html HTTP/1.1\n   
	i = 0;
	j = 0;
	//解析客户端的http请求报文
...
/*
  CGI是外部应用程序（CGI程序）与WEB服务器之间的接口标准执行CGI脚本,在这里就是web服务器和index.html交互
  execute_cgi函数负责将请求传递给cgi程序处理,服务器与cgi之间通过管道pipe
...
void execute_cgi(int client, const char *path, const char *method, const char *query_string)
{
	...
	{
		numchars = get_line(client, buf, sizeof(buf));//注意这里只读完 header 的内容，body 的内容没有读
		//获取HTTP消息实体的传输长度
		////这个循环的目的是读出指示 body 长度大小的参数，并记录 body 的长度大小。其余的 header 里面的参数一律忽略
		//  
		while ((numchars>0) && strcmp("\n", buf))  //循环读每一行，直到遇到Content-Length
		{
			buf[15] = '\0';  //Content-Length: 加\0
	...
	if (pid == 0)
	{
		...
		//第一参数path字符指针所指向要执行的文件路径， 接下来的参数代表执行该文件时传递的参数列表
		//最后一个参数须用空指针NULL作结束。
...
{
	char buf[1024];
	//每次读取一行，函数成功将返回buf，失败或读到文件结尾返回NULL。
	//因此我们不能直接通过fgets的返回值来判断函数是否是出错而终止的，应该借助feof函数或者ferror函数来判断。
	fgets(buf, sizeof(buf), resource);//从文件结构指针resource中读取数据,保存至buf中
									  //处理文件流中剩下的字符
	while (!feof(resource))//检测流上的文件结束符,文件结束返回1值,未结束返回0
	{
		send(client, buf, strlen(buf), 0);//文件流中的字符全部发送给client
		fgets(buf, sizeof(buf), resource);
		/*从文件结构体指针resource中读取至多bufsize-1个数据(第bufsize个字符赋'\0')每次读取一行,
		如果不足bufsize,则读完该行结束.这里通过feof函数来判断fgets是否因出错而终止.
